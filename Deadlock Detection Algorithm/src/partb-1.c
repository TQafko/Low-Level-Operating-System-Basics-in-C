 ...
 */
void handleReleaseAll(struct Lock locks[], int cid) {
 ...
 */
void handleRequest(struct Lock locks[], int cid, int t_lock, int respond_fd) {
    struct msg_requestStatus STAT; /* message from parent to child (me) */
    for (int li = 0; li < MAXLOCKS; li++) {
        if(li == t_lock){
            // check for the lock requested
        ...
        for (int cid = 0; cid < NPROC; cid++) {
            ...
            if (nb == sizeof(ChildRequest)) {
                if (ChildRequest.Action == RELEASE_ALL) {
                    handleReleaseAll(locks, cid);
                } else if (ChildRequest.Action == LOCK) {
                    handleRequest(locks, cid, ChildRequest.lockID, respond[cid]);
                }
            }
        }
    }
